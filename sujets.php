...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Nouveau sujet ajouté avec succès";
    } else {
...
if (isset($_GET['delete'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "Sujet supprimé avec succès";
    } else {
        echo "Erreur: " . $sql . "<br>" . $conn->error;
    }
}
...
<body class="bg-gray-100 text-gray-900 flex flex-col min-h-screen">
    ...
    <h1 class="text-3xl font-bold mb-4">Sujets</h1>
                    ...
                    <div class="mt-8 flex flex-col items-center">
                        <div class="w-full flex-1">
                            <div class="mx-auto max-w-xs">
                                <form method="post" action="sujets.php" class="flex flex-col items-center">
                                    <input class="w-full px-8 py-4 mt-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white" type="text" name="titre" placeholder="Titre du sujet" required />
    ...
    <div class="container mx-auto p-4">
        <h1 class="text-3xl font-bold mb-4">Liste des Sujets</h1>
        <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead>
                <tr>
                    <th class="px-4 py-2 bg-gray-200 text-left text-sm font-semibold text-gray-700">ID</th>
                    <th class="px-4 py-2 bg-gray-200 text-left text-sm font-semibold text-gray-700">Titre</th>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) : ?>
                    <tr>
                        ...
                        <td class="border-t px-4 py-2"><?php echo $row['titre']; ?></td>
                        <td class="border-t px-4 py-2">
                            <a href="sujets.php?delete=<?php echo $row['id']; ?>" class="text-red-500 hover:underline">Supprimer</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
